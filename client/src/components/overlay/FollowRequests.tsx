...
const FollowRequests: React.FC<IProps> = ({ openFollowRequestsOverlay, setOpenFollowRequestsOverlay, followRequestOverlayRef }) => {
  return (
    <div className={`fixed top-0 left-0 bottom-0 right-0 bg-[rgba(0,0,0,.8)] flex items-center justify-center z-20 ${openFollowRequestsOverlay ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} transition`}>
      <div ref={followRequestOverlayRef} className={`w-1/3 bg-white rounded-md ${openFollowRequestsOverlay ? 'translate-y-0' : '-translate-y-16'} transition`}>
