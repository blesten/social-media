...
const Followings: React.FC<IProps> = ({ openFollowingsOverlay, setOpenFollowingsOverlay, followingsOverlayRef }) => {
  return (
    <div className={`fixed top-0 left-0 bottom-0 right-0 bg-[rgba(0,0,0,.8)] flex items-center justify-center z-20 ${openFollowingsOverlay ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} transition`}>
      <div ref={followingsOverlayRef} className={`w-1/3 bg-white rounded-md ${openFollowingsOverlay ? 'translate-y-0' : '-translate-y-16'} transition`}>
