...
const Delete: React.FC<IProps> = ({ openDeleteOverlay, setOpenDeleteOverlay, deleteOverlayRef, onSuccess }) => {
  ...
  return (
    <div className={`fixed top-0 left-0 right-0 bottom-0 z-20 bg-[rgba(0,0,0,.8)] flex items-center justify-center ${openDeleteOverlay ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} transition`}>
      <div ref={deleteOverlayRef} className={`w-1/3 bg-white rounded-md flex flex-col items-center justify-center py-7 ${openDeleteOverlay ? 'translate-y-0' : '-translate-y-16'} transition`}>
