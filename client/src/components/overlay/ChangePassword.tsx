...
const ChangePassword: React.FC<IProps> = ({ openChangePasswordOverlay, setOpenChangePasswordOverlay, changePasswordOverlayRef }) => {
  ...
  return (
    <div className={`fixed top-0 left-0 bottom-0 right-0 bg-[rgba(0,0,0,.8)] flex items-center justify-center z-20 ${openChangePasswordOverlay ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} transition px-5`}>
      <div ref={changePasswordOverlayRef} className={`bg-white transition rounded-md ${openChangePasswordOverlay ? 'translate-y-0' : '-translate-y-16'} md:w-1/3 w-full`}>
        ...
        <form onSubmit={handleSubmit} className='px-5 py-3'>
          <div className='mb-5'>
            ...
            <div className='border border-gray-300 rounded-md p-3 mt-2 flex items-center justify-between'>
              <input type={showCurrentPassword ? 'text' : 'password'} id='currentPassword' name='currentPassword' value={passwordData.currentPassword} onChange={handleChange} className='outline-none text-sm w-full' />
              {
                showCurrentPassword
          ...
          <div className='mb-5'>
            ...
            <div className='border border-gray-300 rounded-md p-3 mt-2 flex items-center justify-between'>
              <input type={showNewPassword ? 'text' : 'password'} id='newPassword' name='newPassword' value={passwordData.newPassword} onChange={handleChange} className='outline-none text-sm w-full' />
              {
                showNewPassword
          ...
          <div className='mb-5'>
            ...
            <div className='border border-gray-300 rounded-md p-3 mt-2 flex items-center justify-between'>
              <input type={showNewPasswordConfirmation ? 'text' : 'password'} id='newPasswordConfirmation' name='newPasswordConfirmation' value={passwordData.newPasswordConfirmation} onChange={handleChange} className='outline-none text-sm w-full' />
              {
